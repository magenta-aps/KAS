...
class Job(models.Model):
    ...
    job_type = models.TextField(
        choices=((k, v["label"]) for k, v in get_job_types().items())
    )
    ...
    def get_rq_job(self):
        queue = django_rq.get_queue(
            self.queue, connection=redis_cursor
        )  # reuse same redis connection
    ...
    def schedule_job(
        ...
        rq_queue = django_rq.get_queue(
            queue, connection=redis_cursor
        )  # reuse same redis connection
        job = cls.objects.create(
            job_type=job_type,
            created_by=created_by,
            parent=parent,
            arguments=job_kwargs,
            queue=queue,
        )
        if depends_on:
    ...
    def push_metrics(self):
        ...
        if settings.DEBUG is False and settings.METRICS["disable"] is False:
            ...
            try:
                push_to_gateway(
                    "pushgateway:9091", job=self.job_type, registry=registry
                )
...
def job_decorator(function):
    ...
    def inner():
        ...
        with transaction.atomic():
            job = Job.objects.select_for_update().filter(uuid=rq_job.meta["job_uuid"])[
                0
            ]
