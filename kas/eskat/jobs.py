...
def importere_kas_beregninger_for_legacy_years(job):
    ...
    gemte_beregninger = 0
    for beregning in SourceModel.values('cpr',
                                        'pension_crt_calc_guid',
                                        'capital_return_tax',
                                        'skatteaar'):
        try:
    ...
    job.result = {'message': 'Fandt {found} beregninger og gemte {saved} beregninger'.format(found=fundet_beregninger,
                                                                                             saved=gemte_beregninger)}
