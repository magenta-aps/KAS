...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        year = options["year"]
        persontaxyear_cprs = set([result['person__cpr'] for result in PersonTaxYear.objects.filter(tax_year__year=year).values('person__cpr')])
        mandtal_cprs = set([result['cpr'] for result in ImportedKasMandtal.objects.filter(skatteaar=year).values('cpr')])
        ...
        if options['perform_delete']:
            ...
            persontaxyear_extraneous = PersonTaxYear.objects.filter(person__cpr__in=extraneous)
            policytaxyear_extraneous = PolicyTaxYear.objects.filter(person_tax_year__in=persontaxyear_extraneous)
