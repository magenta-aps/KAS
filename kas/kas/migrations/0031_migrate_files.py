...
def apply_migration(apps, schema_editor):
    for model, field, path_method in (
            (apps.get_model('kas', 'PensionCompanySummaryFile'), 'file', pensioncompanysummaryfile_file_path_new),
            (apps.get_model('kas', 'PolicyDocument'), 'file', policydocument_file_path_new),
            (apps.get_model('kas', 'TaxSlipGenerated'), 'file', taxslipgenerated_file_path_new),
            (apps.get_model('kas', 'FinalSettlement'), 'pdf', finalsettlement_file_path_new),
            (apps.get_model('prisme', 'PrePaymentFile'), 'file', prepaymentfile_file_path_new),
    ):
...
def revert_migration(apps, schema_editor):
    for model, field, path_method in (
            (apps.get_model('kas', 'PensionCompanySummaryFile'), 'file', pensioncompanysummaryfile_file_path_old),
            (apps.get_model('kas', 'PolicyDocument'), 'file', policydocument_file_path_old),
            (apps.get_model('kas', 'TaxSlipGenerated'), 'file', taxslipgenerated_file_path_old),
            (apps.get_model('kas', 'FinalSettlement'), 'pdf', finalsettlement_file_path_old),
            (apps.get_model('prisme', 'PrePaymentFile'), 'file', prepaymentfile_file_path_old),
    ):
        move_files(model, field, path_method)
    remove_empty_folders(settings.MEDIA_ROOT, True)
