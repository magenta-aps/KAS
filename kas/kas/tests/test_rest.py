...
class RestTest(TestCase):
    ...
    invalid_cpr_values = invalid_string_values + [
        "",
        "hephey",
        "123456-7890",
        "123456789",
    ...
    def setUpClass(cls):
        ...
        cls.user = User.objects.create_user(
            username="tester", email="test@example.com", password="we_are_testing"
        )
            ...
            [
                {
                    "year": 2020,
                    "id": year2020.id,
                    "days_in_year": year2020.days_in_year,
                    "year_part": year2020.year_part,
                }
            ],
            response.json(),
        ...
        self.assertCountEqual(
            [
                ...
                },
                {
                    "year": 2021,
                    "id": year2021.id,
                    "days_in_year": year2021.days_in_year,
                    "year_part": year2021.year_part,
                },
            ],
            response.json(),
...
class PensionCompanyTest(RestTest):
    ...
    def test_get_all(self):
        pension_company_data1 = {
            "res": "12345678",
            "name": "Foobar A/S",
            "address": "Foobarvej 42",
        }
        pension_company_data2 = {
            "res": "12345670",
            "name": "Hephey A/S",
            "address": "Hepheyvej 21",
        }
...
class PersonTaxYearTest(RestTest):
    ...
    def test_invalid_input(self):
        ...
        inputs = (
            ...
            + [
                {**full, "tax_year": x}
                for x in self.invalid_fk_values + [tax_year.id + 1]
            ]
        )
        for input in inputs:
            response = self.client.post(
...
class PolicyTaxYearTest(RestTest):
    ...
    def test_get_all(self):
        ...
        pension_company = PensionCompany.objects.create(
            res=12345678, name="Foobar A/S", address="Foobarvej 42"
        )
    ...
    def test_invalid_input(self):
        ...
        inputs = (
            ...
            + [
                {**full, "preliminary_paid_amount": x}
                for x in self.invalid_positive_amount_values
            ]
...
class PolicyDocumentTest(RestTest):
    ...
    def test_upload(self):
        ...
        with open(filename, "rb") as fp:
            ...
            upload_file = SimpleUploadedFile(
                "textfile.txt", b, content_type="text/plain"
            )
            response = self.client.post(
                self.url,
                {
    ...
    def test_invalid_input(self):
        ...
        inputs = (
            [
                {**full, "policy_tax_year": x}
                for x in self.invalid_fk_values + [policy_tax_year.id + 1]
            ]
        ...
        for input in inputs:
            if len(
                [v for v in input.values() if v in [{}, None]]
