...
class AutoligningTestCase(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.person_tax_year = PersonTaxYear.objects.create(
            tax_year=cls.year, person=cls.person, number_of_days=0
        )
    ...
    def test_created_by_citizen(self, djang_rq):
        ...
        policy_tax_year._history_user = (
            self.rest_user
        )  # fake creating through the res API
        ...
        self.assertEqual(
            PolicyTaxYear.objects.filter(
                person_tax_year=self.person_tax_year,
                slutlignet=False,
                efterbehandling=True,
            ).count(),
            1,
        )
    ...
    def test_changed_by_citizen(self, djano_rq):
        ...
        self.policy_tax_year._history_user = (
            self.rest_user
        )  # citizen changes self_reported_amount
        self.policy_tax_year.save()
        Job.schedule_job(
            autoligning,
            job_type="autoligning",
    ...
    def test_notes_exists(self, django_rq):
        Note.objects.create(
            person_tax_year=self.person_tax_year, author=self.clerk_user, content="text"
        )
        Job.schedule_job(
            autoligning,
            job_type="autoligning",
