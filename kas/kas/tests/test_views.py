...
class SelfReportedAmountUpdateViewTestCase(BaseTestCase):
    def test_not_logged_in(self):
        r = self.client.get(
            reverse("kas:change-self-reported-amount", args=[self.policy_tax_year.pk])
        )
    ...
    def test_valid_get(self):
        ...
        r = self.client.get(
            reverse(
                "kas:change-self-reported-amount",
                kwargs={"pk": self.policy_tax_year.pk},
            )
        )
    ...
    def test_invalid_urls(self):
        ...
        r = self.client.get(
            reverse(
                "kas:change-self-reported-amount", args=[self.policy_tax_year.pk + 1]
            )
        )
    ...
    def test_submit_document(self):
        ...
        r = self.client.post(
            ...
            data={
                ...
                "uploads-0-file": SimpleUploadedFile(
                    name="test", content=b"test_submit_document"
                ),
                "uploads-INITIAL_FORMS": 0,
            },
        )
    ...
    def test_submit_note_and_document(self):
        ...
        r = self.client.post(
            ...
            data={
                ...
                "uploads-0-description": "test_description",
                "uploads-0-file": SimpleUploadedFile(
                    name="test", content=b"test_submit_note_and_document"
                ),
                "uploads-INITIAL_FORMS": 0,
            },
        )
...
class EditAmountsUpdateViewTestCase(BaseTestCase):
    ...
    def test_not_logged_in(self):
        self.client.logout()
        r = self.client.get(
            reverse("kas:change-edit-amounts", args=[self.policy_tax_year.pk])
        )
        # should redirect to login page
        self.assertEqual(r.status_code, 302)
    ...
    def test_set_efterbehandling_by_attachment(self):
        r = self.client.post(
            ...
            data={
                ...
                "uploads-0-description": "test_description",
                "uploads-0-file": SimpleUploadedFile(
                    name="test", content=b"test_set_efterbehandling_by_attachment"
                ),
            },
        )
        self.assertEqual(r.status_code, 200)
...
class PolicyNotesAndAttachmentsViewTestCase(BaseTestCase):
    ...
    def test_not_logged_in(self):
        ...
        r = self.client.get(
            reverse(
                "kas:policy_add_notes_or_attachement", args=[self.policy_tax_year.pk]
            )
        )
        # should redirect to login page
        self.assertEqual(r.status_code, 302)
        ...
        self.client.post(
            ...
            data={
                "attachment": SimpleUploadedFile(
                    name="test", content=b"test_save_attachment"
                ),
    ...
    def test_change_processing_date(self):
        self.policy_tax_year.next_processing_date = (
...
class PersonNotesAndAttachmentsViewTestCase(BaseTestCase):
    ...
    def test_not_logged_in(self):
        ...
        r = self.client.get(
            reverse(
                "kas:person_add_notes_or_attachement", args=[self.person_tax_year.pk]
            )
        )
        # should redirect to login page
        self.assertEqual(r.status_code, 302)
    ...
    def test_save_attachment_and_note(self):
        ...
        self.client.post(
            ...
            data={
                "attachment": SimpleUploadedFile(
                    name="test", content=b"test_save_attachment_and_note"
                ),
                "attachment_description": attachment_description,
                "note": note_string,
            },
...
class PaymentOverrideTestCase(BaseTestCase):
    ...
    def test_no_override(self):
        ...
        self.client.post(
            reverse("kas:policy_payment_override", args=[self.policy_tax_year.pk]),
            follow=True,
            data={"citizen_pay_override": False, **self.management_form_data},
        )
        policy_tax_year = PolicyTaxYear.objects.get(pk=self.policy_tax_year.pk)
    ...
    def test_override(self):
        ...
        self.client.post(
            reverse("kas:policy_payment_override", args=[self.policy_tax_year.pk]),
            follow=False,
            data={"citizen_pay_override": True, **self.management_form_data},
        )
        policy_tax_year = PolicyTaxYear.objects.get(pk=self.policy_tax_year.pk)
    ...
    def test_generate_final_taxslip(self):
        ...
        collect_date = collect_date.replace(
            day=1, month=((collect_date.month + 3) % 12) + 1
        )
...
class CreateLockForYearTestCase(BaseTestCase):
    ...
    def test_create_new_lock(self):
        self.assertFalse(self.tax_year.locks.exclude(interval_to__isnull=True).exists())
        r = self.client.post(
            reverse("kas:lock-create"), data={"taxyear": self.tax_year.pk}, follow=True
        )
        self.assertEqual(r.status_code, 200)
        ...
        # 1 open lock
        self.assertEqual(
            self.tax_year.locks.filter(interval_to__isnull=True).count(), 1
        )
        # 1 closed lock
        self.assertEqual(
            self.tax_year.locks.exclude(interval_to__isnull=True).count(), 1
        )
