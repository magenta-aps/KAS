...
class GroupPermissionTestCase(TestCase):
    ...
    def test_view_permissions_for_kas(self):
        ...
        view_permissions = (
            Permission.objects.filter(content_type__in=kas_content_types)
            .filter(codename__startswith="view_")
            .exclude(content_type__model="pensioncompanysummaryfile")
        )
        for group in Group.objects.all():
            group_view_permission = (
                group.permissions.filter(content_type__in=kas_content_types)
                .filter(codename__startswith="view_")
                .exclude(content_type__model="pensioncompanysummaryfile")
            )
    ...
    def test_administrator_worker_permission(self):
        ...
        self.assertTrue(
            set(group.permissions.filter(content_type__in=content_types)).issubset(
                set(permissions)
            )
        )
    ...
    def test_sagsbehandler_kas_permission(self):
        ...
        expected_kas_permission = (
            Permission.objects.filter(content_type__in=kas_content_types)
            .exclude(codename__startswith="delete_")
            .exclude(Q(codename="add_taxyear") | Q(codename="change_taxyear"))
        )
        current_group_permissions = group.permissions.filter(
            content_type__in=kas_content_types
        )
    ...
    def test_borger_service_kas_permissions(self):
        ...
        expected_kas_permission = (
            ...
            .filter(
                ...
                | Q(codename__in=["add_policydocument", "add_note"])
            )
            .exclude(content_type__model="pensioncompanysummaryfile")
        )
        current_group_permissions = group.permissions.filter(
    ...
    def test_regnskab_prisme_permissions(self):
        ...
        expected_prism_permission = Permission.objects.filter(
            content_type__in=prisme_content_types
        ).filter(Q(codename__startswith="view_") | Q(codename="add_prisme10qbatch"))
        current_group_permissions = group.permissions.filter(
            content_type__in=prisme_content_types
        )
