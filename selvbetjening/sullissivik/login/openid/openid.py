...
class OpenId:
    ...
    whitelist = [
        reverse_lazy("sullissivik:openid:login"),
        reverse_lazy("sullissivik:openid:login-callback"),
        reverse_lazy("sullissivik:openid:logout-callback"),
    ]
    ...
    def logout(cls, session):
        ...
        auth_req = client.construct_EndSessionRequest(
            request_args=request_args, id_token=session["access_token_data"]["id_token"]
        )
