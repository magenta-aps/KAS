...
class LoginManager:
    ...
    def white_listed_urls(self):
        return (
            OpenId.whitelist
            + settings.LOGIN_REQUIREMENT_WHITELIST
            + [
                reverse("sullissivik:openid:login"),
    ...
    def __call__(self, request):
        if self.enabled:
            ...
            if (
                request.path not in white_listed_urls
                and request.path.rstrip("/") not in white_listed_urls
            ):
                if (
                    "user_info" not in request.session
                    or not request.session["user_info"]
                ):
                    if not self.authenticate(
                        ...
                        return redirect(
                            reverse_lazy(settings.LOGIN_UNAUTH_REDIRECT)
                            + "?back="
                            + backpage
                        )
    ...
    def get_backpage(request):
        backpage = request.GET.get(
            "back",
            request.session.get("backpage", reverse(settings.LOGIN_DEFAULT_REDIRECT)),
        )
        return backpage
